...
import { Eye, EyeOff, Lock, ShieldCheck, Wifi, WifiOff, Wrench } from 'lucide-react';
import { NeoButton } from '../components/ui/NeoButton';
import toast from 'react-hot-toast';
import { auth, db } from '../firebase';
import { getIdTokenResult, sendPasswordResetEmail, signInWithEmailAndPassword, signOut } from 'firebase/auth';
...
const AdminLogin = () => {
  ...
  const canSubmit = !loading && emailLooksValid && Boolean(password);
  ...
  const handleLogin = async (event: React.FormEvent) => {
    ...
    try {
      ...
      if (!tokenResult.claims.admin) {
        await signOut(auth);
        toast.error('Admin access is managed by Firebase custom claims. This account is not approved.');
        return;
      }
    ...
    <div className="min-h-screen bg-bg flex items-center justify-center p-4">
      <div className="bg-white border-4 border-dark rounded-3xl p-8 shadow-neo max-w-md w-full text-center relative overflow-hidden">
        ...
        <p className="text-gray-600 font-bold mb-3 flex items-center justify-center gap-2">
          <Lock size={16} /> Custom-claim protected area
        </p>
        ...
        <form onSubmit={handleLogin} className="space-y-4">
          ...
          <div className="flex items-center justify-between text-xs font-bold">
            ...
            <span className="text-gray-400">Firebase rules + claims</span>
          </div>
        ...
        <div className="mt-6 flex items-center justify-center gap-2 text-xs text-gray-400 font-bold">
          <Wrench size={12} /> Use the admin-claim script or Admin SDK to grant access
        </div>
      </div>
    </div>
  );
