...
const shellArgs = isWindows
  ? [
      '-NoProfile',
      '-Command',
      `& '${firebaseBin}' emulators:start --project ${projectId} --only auth,firestore,functions`,
    ]
  : ['-lc', `"${firebaseBin}" emulators:start --project ${projectId} --only auth,firestore,functions`];
